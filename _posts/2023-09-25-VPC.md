...
- Controls how traffic is allowed into our outo EC2 instances
- SGs are Stateful (Requires a rule only in one directio)
- Outcoming traffic (from the VPC) first hits the SG and after, the NACL
- SGs can reference other SGs in their rules
- All inbould traffic is blocked by default

## Network Access Control List (NACL)

- It is attached to a Subnet
- Controls how traffic is allowed into a Subnet
- SGs are Stateless (Requires rules in both directions)
- Incoming traffic (from Internet) first hits the NACL and after, the SG
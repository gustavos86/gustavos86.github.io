...
- EC2 instances are attached to a single subnet, therefore attached to a single AZ
...
The diagram shows 2 EC2 instances to different AZs. Single both Subnets are attached to the single Route table the communication local communication is possible.
...
Additionally to the Main Route table, a Custome Route table can bs created.
Since the Custom Route table does not have a 0.0.0.0/0 route to the Internet Gateway (IGW), the EC2 instance with IP address 172.31.1.11 does not have Internet access.
...
- 172.31.0.3/24 - Reserve dby AWS for future use
